...
export default function App() {
  ...
  const processTranslation = async (text) => {
    ...
    try {
      ...
      if (response) {
        ...
        // Guardar asÃ­ncronamente en PostgreSQL sin bloquear el UI
        saveTranslation({
          original_text: text,
      ...
      setError('Error al procesar la traducciÃ³n. Intenta de nuevo.');
    } finally {
  ...
  const speakText = (text, languageName) => {
    ...
    if(lowerLang.includes('spanish') || lowerLang.includes('espaÃ±ol')) {
      langCode = 'es-ES';
    } else if(lowerLang.includes('german') || lowerLang.includes('alemÃ¡n') || lowerLang.includes('deutsch')) {
      langCode = 'de-DE';
    }
